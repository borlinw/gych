...
function createMenu(_xtype, _json) {// 左侧菜单生成
	...
	case 'LeftMenu':
		...
		var html = "<div id='"+ id+ "' style='display:none;float:left;width:180px;heigth:52px;margin:5px 0 0 15px;'>" +
				"<a href='"+href+"' target='rightContent' style='text-decoration:none;'><div style='float:left; display:inline;margin-right:5px;'><img style='border:none;' src='"
...
function loadDist(id, dwbm) {
	$('#' + id)
			.combotree(
					{
						checkbox : true,
						url : '/gych/xtgl/selAllXzqh.do?yhdw=' + dwbm,
						onBeforeExpand : function(node, param) {
							$('#' + id).combotree("tree").tree('options').url = "/gych/xtgl/selAllXzqh2.do?yhdw="
									+ node.id;
						},
	...
	$('#' + id).combotree(
			{
				...
				multiple:true,
				async:false,
				url : '/gych/gcgl/selAllXzqh1.do?yhdw=' + dwbm,
	...
	$('#' + id).combotree(
			{
				...
				multiple:true,
				async:false,
				url : '/gych/gcgl/selAllUnit1.do?yhdw=' + dwbm,
				onLoadSuccess : function (node){
					$('#' + id).combotree('setValue', dwbm);
				}
		});
...
function loadUnit(id, dwbm) {
	$('#' + id).combotree({
		checkbox : false,
		url : '/gych/xtgl/selAllBm.do?yhdw=' + dwbm,
		onBeforeExpand : function(node, param) {
			$('#' + id).combotree("tree").tree('options').url = "/gych/xtgl/selAllBm2.do?yhdw="
					+ node.id;
		},
		onSelect : function(node) {
			YMLib.Var.DistName = node.text
...
function loadBmbm(id, name) {
	$('#' + id).combobox({
		url : '/gych/xtgl/getBmbmTreeByName2.do?yhm='
...
function loadBmbm2(id, name) {
	$.ajax({
		type:'post',
		async:false,
		url:'/gych/xtgl/getBmbmTreeByName2.do',
...
function locationQl(_roadbm,_roadpos){
	...
	YMLib.Var.bm=data;
	YMLib.Var.type="2";
	YMLib.UI.createWindow('locationXm','项目定位','/gych/openlayers.html','app_add',800,500);
...
function locationXm(xmbm,jdbs){
	$.ajax({
		type:'post',
		url:'/gych/qqgl/selectlxbyxmid.do',
...
function createMenunew(id){
	$("#tree"+id).tree({
        //data : treeData,
		url:'/gych/xtgl/createMenu.do?parent='+id,
        lines : true,
        onClick : function (node) {
            if (node.attributes.length!=0) {
                //alert(node.attributes);
            	$.messager.alert('id',node.id);
...
function getQxfromSession(qx){
	...
	$.ajax({
		...
		dateType:'json',
		async:false,
		url:'/gych/xtgl/getQxfromSession.do',
		success:function(msg){
			qxstrng=msg;
		}
	});
